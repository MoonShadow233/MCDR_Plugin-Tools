...
def register_tools_commands(server: PluginServerInterface):
    ...
    """
    from mcdreforged.api.command import Literal, Text, Integer, Float, Number, QuotableText, Enumeration, GreedyText
    ...
    if settings.get('enable_here', True):
        register_here_command(server)
    
    if settings.get('enable_position', True):
        register_position_command(server)
    
    if settings.get('enable_tp', True):
        register_gamemode_tp_command(server)
    
    if settings.get('enable_scale', True):
        register_scale_command(server)
    
    if settings.get('enable_music', True):
        register_music_command(server)
    
    server.logger.info("Tools commands registered successfully!")

def register_here_command(server: PluginServerInterface):
    """注册 !here / !h 命令"""
    def callback(source: CommandSource, context: CommandContext):
        if source.is_player:
            info = create_fake_info(source)
            Here(server, info).GetPos()
    
    root = Literal('!here').runs(callback)
    server.register_command(root)
    
    root2 = Literal('!h').runs(callback)
    server.register_command(root2)

def register_position_command(server: PluginServerInterface):
    """注册 !d 位置传送命令"""
    pos_manager = Position(server)
    
    def list_callback(source: CommandSource, context: CommandContext):
        if source.is_player:
            info = create_fake_info(source)
            page = context.get('page', 1)
            pos_manager.getpositionlist(info, page)
    
    def tp_callback(source: CommandSource, context: CommandContext):
        if source.is_player:
            info = create_fake_info(source, f"!d tp {context.get('name', '')}")
            args = info.content.split()
            pos_manager._teleport_to_position(info, args)
    
    def set_callback(source: CommandSource, context: CommandContext):
        if source.is_player:
            info = create_fake_info(source, f"!d set {context.get('name', '')}")
            pos_manager.set_location(info)
    
    root = Literal('!d')
    list_node = Literal('list')
    list_node.then(Integer('page').at_min(1).runs(list_callback))
    list_node.runs(list_callback)
    root.then(list_node)
    
    tp_node = Literal('tp').then(QuotableText('name').runs(tp_callback))
    root.then(tp_node)
    
    set_node = Literal('set').then(QuotableText('name').runs(set_callback))
    root.then(set_node)
    
    server.register_command(root)

def register_gamemode_tp_command(server: PluginServerInterface):
    """注册 !tp 旁观传送命令"""
    tp_manager = GamemodeTp(server)
    
    def callback(source: CommandSource, context: CommandContext):
        if source.is_player:
            info = create_fake_info(source, context.get('__raw', '!tp'))
            tp_manager.get_player_info(info)
    
    root = Literal('!tp')
    root.then(Float('x').then(
        Float('y').then(Float('z').runs(callback))
    ).then(Float('z').runs(callback)))
    root.then(Text('player_or_dim').runs(callback))
    root.runs(callback)
    
    server.register_command(root)

def register_scale_command(server: PluginServerInterface):
    """注册 !sc 缩放命令"""
    scale_manager = Scale(server)
    
    def callback(source: CommandSource, context: CommandContext):
        if source.is_player:
            info = create_fake_info(source, f"!sc {context.get('scale', 1)}")
            scale_manager.scale(info)
    
    root = Literal('!sc')
    root.then(Number('scale').at_min(0.01).at_max(100).runs(callback))
    root.runs(callback)
    
    server.register_command(root)

def register_music_command(server: PluginServerInterface):
    """注册 !music 音乐命令"""
    def callback(source: CommandSource, context: CommandContext):
        if source.is_player:
            info = create_fake_info(source, context.get('__raw', '!music'))
            Music(server, info).PlayMusic()
    
    root = Literal('!music')
    root.then(Literal('stop').runs(callback))
    root.then(Literal('p').then(Literal('stop').runs(callback)))
    root.then(Literal('p').then(Literal('id').then(Text('music_id').runs(callback))))
    root.then(Literal('p').then(Literal('name').then(Text('music_name').runs(callback))))
    root.runs(callback)
    
    server.register_command(root)

def create_fake_info(source: CommandSource, content: str = ''):
    """创建一个假的 Info 对象用于命令回调"""
    from mcdreforged.info_reactor.info import Info, InfoSource
    info = Info(InfoSource.PLAYER, content)
    info._player = source.player
    return info