...
class GamemodeTp():
    ...
    def get_player_info(self, info):
        ...
        # self.server.tell(info.player, f"你的游戏模式为: {gamemode}") 
        player_dim = api.get_player_dimension(info.player) 
        # self.server.tell(info.player, f"你的维度: {player_dim}") 
        if gamemode == '3':
            if len(args) > 4:
                self.tpdebug(info)
                return
            elif str(len(args)) == str(4):
                x = args[1]
                y = args[2]
                z = args[3]
                try:
                    ...
                    self.server.tell(info.player, '§c你确定你输入的是数字吗？（必须是双浮点double）') 
                    self.tpdebug(info)
                    return
                self.server.tell(info.player, f"§c你正在尝试传送到 {x} {y} {z}##2") 
                player_dim = api.get_player_dimension(info.player) 
                if player_dim == 0:
                    self.server.execute(f'execute in minecraft:overworld run tp {info.player} {x} {y} {z}')
                elif player_dim == -1:
                    self.server.execute(f'execute in minecraft:the_nether run tp {info.player} {x} {y} {z}')
                elif player_dim == 1:
                    self.server.execute(f'execute in minecraft:the_end run tp {info.player} {x} {y} {z}')
                else:
                    self.server.tell(info.player, '§c无法识别的维度，请联系管理员#101') 
            elif str(len(args)) == str(3):
                ...
                self.server.execute(f'execute in minecraft:overworld run tp {info.player} 0 64 0')
            elif args[1] == '地狱' or args[1] == '下界' or args[1] == '下届':
                self.server.execute(f'execute in minecraft:the_nether run tp {info.player} 0 120 0')
            elif args[1] == '末地':
                self.server.execute(f'execute in minecraft:the_end run tp {info.player} 0 64 0')
            elif str(len(args)) == str(2):
                tptarget = args[1]
                self.server.tell(info.player, f"§c你正在尝试传送到玩家 {tptarget}##4") 
                self.server.execute(f'tp {info.player} {tptarget}')
    ...
    def tpdebug(self, info: Info):
        ...
        # self.server.tell(info.player, '§c============================================') 
        # self.server.tell(info.player, '§c!tp旁观传送 By MoonShadow233') 
